...
var Neuroevolution = function(options) {
  ...
  /**
	 ...
	 * @constructor
	 */
  var Generation = function(previousData) {
    if (previousData) {
      this.id = previousData.id
      this.genomes = previousData.genomes
      this.createdAt = previousData.createdAt
    } else {
  ...
  Generation.prototype.toJSON = function() {
    return {
      id: this.id,
  ...
  Generations.prototype.saveCurrent = function() {
    store.set("generations_" + this.currentGeneration.id, this.currentGeneration.toJSON())
  }

  Generations.prototype.sync = function() {
    var results = []
    store.each(function(value, key) {
      if (key.indexOf("generations_") === 0 && value) {
        results.push(value)
      }
    })
  ...
  Generations.prototype.nextGeneration = function() {
    ...
    if (this.generations.length === 0) {
      console.error("not current generations")
      return
    }
    this.saveCurrent()
    this.currentGeneration = new Generation()
    return this.generations[this.generations.length - 1].generateNextGeneration()
  }
  ...
  Generations.prototype.addGenome = function(genome) {
    if (!this.currentGeneration) {
      console.error("missing currentGeneration")
      return
    }
  ...
  self.nextGeneration = function() {
    ...
    if (!self.generations.currentGeneration) {
  ...
  self.totalGenerations = function() {
    return this.generations.generations.length || 0
  }
